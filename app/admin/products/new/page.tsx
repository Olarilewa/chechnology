            ...
            <div className="glass-card border border-white/8 p-8 rounded-2xl">
                <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
                    <div className="grid sm:grid-cols-2 gap-5">
                        ...
                        <FormField label="Slug" required hint="URL: /products/this-slug">
                            <input type="text" className="form-input font-mono text-sm"
                                {...register('slug', { required: true })} />
                        </FormField>
                    </div>

                    <FormField label="Tagline" hint="Short memorable phrase">
                        <input type="text" className="form-input"
                            placeholder="e.g. The platform that connects..." {...register('tagline')} />
                    </FormField>

                    <FormField label="Status" required>
                        <SelectInput
                            options={['concept', 'development', 'beta', 'live', 'deprecated']
                                .map(s => ({ value: s, label: s }))}
                            {...register('status')} />
                    </FormField>

                    <FormField label="Description" hint="2–3 sentences shown on product cards">
                        <TextAreaInput rows={3}
                            placeholder="What does this product do and who is it for?"
                            {...register('description')} />
                    </FormField>

                    <FormField label="Full Content" hint="Detailed content shown on the product page">
                        <TextAreaInput rows={8}
                            placeholder="Full product description, how it works, architecture, vision..."
                            {...register('rich_content')} />
                    </FormField>

                    <div className="grid sm:grid-cols-2 gap-5">
                        <FormField label="Technology Stack" hint="Comma-separated">
                            <input type="text" className="form-input"
                                placeholder="Next.js, Supabase, Python" {...register('technology_stack')} />
                        </FormField>
                        <FormField label="Engineering Disciplines" hint="Comma-separated">
                            <input type="text" className="form-input"
                                placeholder="Software Engineering, AI" {...register('engineering_disciplines')} />
                        </FormField>
                    </div>

                    <FormField label="Industries" hint="Comma-separated">
                        <input type="text" className="form-input"
                            placeholder="FinTech, EdTech" {...register('industries')} />
                    </FormField>
                    ...
                    <div className="grid sm:grid-cols-2 gap-5">
                        <FormField label="GitHub URL">
                            <input type="url" className="form-input"
                                placeholder="https://github.com/..." {...register('github_url')} />
                        </FormField>
                        <FormField label="Documentation URL">
                            <input type="url" className="form-input"
                                placeholder="https://docs...." {...register('documentation_url')} />
                        </FormField>
                    </div>
                    ...
                    <div className="grid sm:grid-cols-2 gap-5">
                        <FormField label="SEO Title">
                            <input type="text" className="form-input"
                                placeholder="Product Name — Chechnology" {...register('seo_title')} />
                        </FormField>
                        <FormField label="SEO Description">
                            <input type="text" className="form-input"
                                placeholder="160 chars max" {...register('seo_description')} />
                        </FormField>
                    </div>
                    ...
                    {error && (
                        <p className="text-sm text-red-400 p-3 rounded-xl bg-red-400/10 border border-red-400/20">
                            {error}
                        </p>
                    )}
